...
def push_frame(cfg: dict, camera: RaceSpyCamera) -> dict | None:
    """
    Capture a JPEG and POST it to /api/cameras/{camera_id}/frame.
    Returns the assignment dict from the server, or None on failure.
    Response: {"status": "pending"|"assigned", "role": ..., "event_id": ...}
    """
    ...
    try:
        ...
        if r.status_code == 200:
            return r.json()
        log.warning("push_frame: HTTP %s", r.status_code)
        return None
    except Exception as exc:
        log.warning("push_frame error: %s", exc)
        return None


def run_preview_mode(cfg: dict, leds: LEDSet, camera: RaceSpyCamera) -> bool:
    """
    Push frames to the server and poll for admin assignment.
    Green LED blinks while waiting. Returns True once assigned.
    """
    log.info("Entering PREVIEW mode — waiting for admin assignment")
    leds.set_setup()   # green blink = waiting for assignment

    while True:
        result = push_frame(cfg, camera)
        if result and result.get("status") == "assigned":
            ...
            if role and event_id:
                ...
                save_config(cfg)
                log.info("Assigned by admin: role=%s event_id=%s", role, event_id)
                return True
        # ~2 fps — fast enough for a usable live preview
        time.sleep(0.5)
